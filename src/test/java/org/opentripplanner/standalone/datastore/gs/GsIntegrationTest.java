...
import static org.opentripplanner.standalone.datastore.gs.GsHelper.toUri;
...
public class GsIntegrationTest {
    ...
    private static final String BUCKET_NAME = "otp-test-bucket";
    ...
    public void testGsDirectory() throws Exception {
        ...
        dir.rename("b.txt", "c.txt");

        content = dir.content();
        // Assert file is moved
        assertEquals(content.toString(), 1, content.size());
        assertTrue(content.toString(), content.toString().contains("my-test-dir/c.txt"));
        ...
        // Test accessing root dir
        CompositeDataSource root = repo.findCompositeSource(
                GsHelper.toUri(BUCKET_NAME, ""),
                FileType.OTP_STATUS
        );
        DataSource rootFile = root.entry("r.txt");
        writeDataToDataSource(rootFile);
        content = root.content();
        assertTrue(content.toString(), content.stream().anyMatch(it -> it.name().equals("r.txt")));
        root.delete("r.txt");
