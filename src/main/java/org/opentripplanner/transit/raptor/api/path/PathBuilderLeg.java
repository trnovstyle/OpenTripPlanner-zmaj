...
public class PathBuilderLeg<T extends RaptorTripSchedule> {
    ...
    static <T extends RaptorTripSchedule> PathBuilderLeg<T> transitLeg(
            ...
            @Nullable RaptorTransferConstraints txConstraintsAfterLeg
    ) {
        return new PathBuilderLeg<>(
                new MyTransitLeg<>(trip, boardAndAlightTime, txConstraintsAfterLeg)
        );
    }
    ...
    @Nullable
    public RaptorTransferConstraints transferConstraintsAfterLeg() {
        return isTransit() ? asTransitLeg().transferConstraintsAfterLeg : null;
    }

    public void setTransferConstraintsAfterLeg(
            @Nullable RaptorTransferConstraints transferConstraintsAfterLeg
    ) {
        var old = asTransitLeg();
        this.leg = new MyTransitLeg<>(old.trip, old.boardAndAlightTime, transferConstraintsAfterLeg);
    }
    ...
    public void changeBoardingPosition(int stopPosition) {
        ...
        this.leg = new MyTransitLeg<>(old.trip, boardAndAlightTime, old.transferConstraintsAfterLeg);
    ...
    private TransitPathLeg<T> createTransitPathLeg(
        ...
        return new TransitPathLeg<>(
                leg.trip, leg.boardAndAlightTime, leg.transferConstraintsAfterLeg, cost, nextLeg
        );
    }
    ...
    private int transitCost(CostCalculator costCalculator, RaptorSlackProvider slackProvider) {
        ...
        var txConstraintsBeforeLeg = prevTransit == null
                ? null
                : prevTransit.transferConstraintsAfterLeg();

        int boardCost = costCalculator.boardingCost(
                prev != null && prev.isAccessWithoutRides(),
                prevStopArrivalTime,
                leg.fromStop(),
                fromTime,
                trip(),
                txConstraintsBeforeLeg
        );
    ...
    private static class MyTransitLeg<T extends RaptorTripSchedule> implements MyLeg {
         ...
         */
        @Nullable
        private final RaptorTransferConstraints transferConstraintsAfterLeg;

        private MyTransitLeg(
                T trip,
                BoardAndAlightTime boardAndAlightTime,
                RaptorTransferConstraints transferConstraintsAfterLeg
        ) {
            this.trip = trip;
            this.boardAndAlightTime = boardAndAlightTime;
            this.transferConstraintsAfterLeg = transferConstraintsAfterLeg;
        }
