...
import org.opentripplanner.transit.raptor.api.transit.RaptorTransferConstraintsSearch;
 ...
 * See {@link #headPathBuilder(RaptorTransferConstraintsSearch, RaptorSlackProvider, CostCalculator)}
 * and {@link #tailPathBuilder(RaptorTransferConstraintsSearch, RaptorSlackProvider, CostCalculator)}
 * <p>
...
public abstract class PathBuilder<T extends RaptorTripSchedule> {
  ...
  @Nullable
  private final RaptorTransferConstraintsSearch<T> transferConstraintsSearch;
  ...
  protected PathBuilder(
          @Nullable RaptorTransferConstraintsSearch<T> transferConstraintsSearch,
  ...
  public static <T extends RaptorTripSchedule> PathBuilder<T> tailPathBuilder(
          RaptorTransferConstraintsSearch<T> transferConstraintsSearch,
          RaptorSlackProvider slackProvider,
          @Nullable CostCalculator costCalculator
  ) {
  ...
  public void transit(
          ...
          RaptorTransferConstraints txConstraintsAfter
  ) {
    add(PathBuilderLeg.transitLeg(trip, times, txConstraintsAfter));
  }
  ...
  private void addTransferConstraints(PathBuilderLeg<T> from, PathBuilderLeg<T> to) {
    ...
    var tx = transferConstraintsSearch.findTransferConstraints(
            from.trip(), from.toStopPos(), to.trip(), to.fromStopPos()
    );
    if(tx != null) { from.setTransferConstraintsAfterLeg(tx); }
  }
