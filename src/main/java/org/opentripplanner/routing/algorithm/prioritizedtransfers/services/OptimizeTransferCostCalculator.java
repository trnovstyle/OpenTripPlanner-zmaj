 ...
 * to calculate waiting time cost. The goal is to maximize the overall waiting time and distribute
 * the waiting time evenly between each transfer.
 * <p>
 * This implementation is a BEST GUESS, we do not have any scientific support for the
 * function used.
 * <p>
...
public class OptimizeTransferCostCalculator {
  ...
  public int cost(Path<?> path) {
    ...
    while (!next.isEgressLeg()) {
      // Wait time including slack
      int waitTime = next.fromTime() - prev.toTime();

      if(next.isTransferLeg()) {
        prev = next;
        next = next.nextLeg();
        cost += prev.generalizedCost();

        waitTime += next.fromTime() - prev.toTime();
      }

      double waitCost = calculateWaitCost(waitTime);
  ...
  /**
   * Unit: cost seconds (not Raptor centi-seconds)
   */
  double calculateWaitCost(int durationSeconds) {
    assertMinSafeTransferTimeSet();
    return n * t0 / (1d + (n-1d) * Math.log1p(a * durationSeconds));
  }
